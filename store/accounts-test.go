...
type mockAccountRepoStore struct {
	...
	result usecases.Account
	err    error
}

func (m mockAccountRepoStore) Create(_ context.Context, _ usecases.Account) (usecases.Account, error) {
...
func (m mockCreateAccountPresenter) Output(_ usecases.Account) CreateAccountOutput {
	return m.result
}
...
func TestCreateAccountInteractor_Execute(t *testing.T) {
	...
	tests := []struct {
		name          string
		args          args
		repository    usecases.AccountRepository
		...
		{
			name: "gerar erro na criação da conta",
			args: args{
				input: CreateAccountInput{
					Name:    "",
					CPF:     "",
